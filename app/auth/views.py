...
from .forms import LoginForm, RegistrationForm, Forgot_passwordForm, New_passwordForm
...
def forgot_password():
	...
	if form.validate_on_submit():
		...
		if user is not None:
			...
			flash('A mail has been sent to your mailbox to reset your password')
			return redirect(url_for('auth.login'))
		else:
			flash('The user is not exist, please register at first')
...
def new_password(token):
	...
	user_id = User.identify_token(token)
	user = User.query.filter_by(id=user_id).first()
	if user is None:
		flash("Invalid user ID! Register at first")
		return redirect(url_for('auth.register'))
